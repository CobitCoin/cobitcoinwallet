...
private:
  ...
  const QDir m_cobitcoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfCobitcoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _cobitcoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo cobitcoinBlockchainInfo() const;
