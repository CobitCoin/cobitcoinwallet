...
const char DEFAULT_WALLET_FILE_NAME[] = "cobitcoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.cobitcoin.io:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("cobitcoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("cobitcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("cobitcoinwallet.desktop");
  ...
  res = autorunSettings.contains("CobitcoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("CobitcoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.cobitcoin.cobitcoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Cobitcoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("CobitcoinWallet", appPath);
    } else {
      autorunSettings.remove("CobitcoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\cobitcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:cobitcoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\cobitcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\cobitcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("cobitcoinwallet.cfg"));
