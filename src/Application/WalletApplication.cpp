...
const char BYTECOIN_URI_SCHEME_NAME[] = "cobitcoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("cobitcoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("cobitcoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Cobitcoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Cobitcoin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have cobitcoind with non-default RPC port, you should terminate it and relaunch CobitcoinWallet\n"
      "or\n"
      "Set the Local deamon required port in CobitcoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
